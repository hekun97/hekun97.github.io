...
```sql
SELECT _name FROM t_student WHERE _fk = (SELECT _fk FROM t_student WHERE _name = "朱军") AND _name != "朱军"
```

::: tip 提示

该条语句更简单，这个也没必要去查班级，直接相同的外建就可以代表同一班级。
